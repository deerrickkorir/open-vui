...
/** Framework-specific props for all components (kept minimal) */
export type ElementProps<TRenderable = unknown> = {
  // Vue template refs are handled by Vue itself and do not flow through props.
  // Keep this loosely typed for TSX / advanced usage.
  ref?: unknown;
};

/** Base type for any renderable constructor */
export type RenderableConstructor<
  TRenderable extends BaseRenderable = BaseRenderable
> = new (ctx: RenderContext, options: any) => TRenderable;
...
> = TOptions & {
  style?: Partial<
    Omit<TOptions, GetNonStyledProperties<RenderableConstructor<TRenderable>>>
  >;
} & ElementProps<TRenderable>;

/** Valid text content types for Text component children */
type TextChildren = string | number | boolean | null | undefined | any;
...
export type AsciiFontProps = ComponentProps<
  ASCIIFontOptions,
  ASCIIFontRenderable
>;

export type TabSelectProps = ComponentProps<
  TabSelectRenderableOptions,
  TabSelectRenderable
> & {
  focused?: boolean;
